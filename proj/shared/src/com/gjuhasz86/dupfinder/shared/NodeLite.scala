...
case class NodeLite(path: String, stats: Stats) {
  def ntype = stats(classOf[NType]).map(_.value).getOrElse("")
  def hash = stats(classOf[Hash]).map(_.value)
  def size = stats(classOf[Size]).map(_.value)
  def name = stats(classOf[Name]).map(_.value)
...
sealed trait Stat
object Stat {
  case class NType(value: String) extends Stat
